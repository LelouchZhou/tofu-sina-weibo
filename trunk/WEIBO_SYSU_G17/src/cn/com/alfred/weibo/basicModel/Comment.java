...
import cn.com.alfred.weibo.json.JSONArray;
...
public class Comment extends WeiboResponse implements java.io.Serializable {
	...
	// write by myself
	private Comment reply_comment; // 评论来源
	private Status status; // 评论的微博
	...
	public Comment(JSONObject json)throws WeiboException, JSONException{
		...
		if(!json.isNull("reply_comment"))
			reply_comment = new Comment(json.getJSONObject("reply_comment"));
		if(!json.isNull("status"))
			status = new Status(json.getJSONObject("status"));
    }

	/* package */Comment(Response res, Element elem, Weibo weibo)
	...
	/* package */
	static List<Comment> constructComments(Response res, Weibo weibo)
			throws WeiboException {
	...
	public String toString() {
		return "Comment{" + "createdAt=" + createdAt + ", id=" + id
